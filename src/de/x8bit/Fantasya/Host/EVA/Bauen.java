...
public class Bauen extends EVABase {

	public Bauen()	{
		super("mache", "Bauen von Gebäuden und Straßen");	// Regionen weise!
    ...
    public static List<BefehlsMuster> getMuster() {
        ...
        bm = new BefehlsMuster(Bauen.class, 11, "^(mache)[n]? (strasse) (" + richtungen + ")([ ]+(\\/\\/).*)?", "m", Art.LANG);
        ...
        bm = new BefehlsMuster(Bauen.class, 21, "^(mache)[n]? " + regEx + "([ ]+(\\/\\/).*)?", "m", Art.LANG);
        bm.setKeywords("mache", "machen");
        retval.add(bm);
        // Variante 22 - Schiff weiterbauen
		bm = new BefehlsMuster(Bauen.class, 22, "^(mache)[n]? (schiff) [a-z0-9]{1,4}([ ]+(\\/\\/).*)?", "m", Art.LANG);
        ...
        bm = new BefehlsMuster(Bauen.class, 31, "^(mache)[n]? " + regEx + "([ ]+(\\/\\/).*)?", "m", Art.LANG);
        bm.setKeywords("mache", "machen");
        retval.add(bm);
        // Variante 32 - Gebaude weiterbauen
        bm = new BefehlsMuster(Bauen.class, 32, "^(mache)[n]? " + regEx + " [a-z0-9]{1,4}([ ]+(\\/\\/).*)?", "m", Art.LANG);
		bm.addHint(new IDHint(2));
