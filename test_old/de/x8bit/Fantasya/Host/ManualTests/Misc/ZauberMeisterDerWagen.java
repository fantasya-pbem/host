 ...
 * @author hb
 */
public class ZauberMeisterDerWagen extends TestBase {
    ...
    protected void mySetupTest() {
...

        {
			...
			p2.setName("Z-MeisterDerWagen");

			Unit magier = this.createMage(p2, r, 6);
			magier.setName(this.getName() + " 02");
			magier.setSpell(new MeisterDerWagen());
			magier.Befehle.add("ZAUBERE \"Meister der Wagen\" " + u.getNummerBase36());
        ...
        for (Unit u:units) {
            ...
            if (tokens[1].equals("01")) {
                ...
                for (Message msg : messages) {
                    ...
                    if (text.contains("durch seine erfahrung") && text.contains("1 wagen mehr")) found = true;
                }
                if (!found) retval = fail(tokens[1] + ": Meldung über magische Hilfe fehlt.");

				if (u.getItem(Wagen.class).getAnzahl() != 10) {
