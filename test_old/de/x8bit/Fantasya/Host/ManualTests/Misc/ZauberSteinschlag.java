 ...
 * @author hb
 */
public class ZauberSteinschlag extends TestBase {
    ...
    protected void mySetupTest() {
...

        {
			...
			p2.setName("Z-Steinschlag");

			Unit magier = this.createMage(p2, r, 6);
			magier.setName(this.getName() + " 02 " + magier.getAura());
			magier.setSpell(new Steinschlag());
			magier.Befehle.add("ZAUBERE \"Steinschlag\" 4");
			...
			// Als Kampfzauber:
			r = this.getTestWorld().nurTerrain(getRegions(), Berge.class).get(0);
			getRegions().remove(r);
			...
			magier.setSpell(new Steinschlag());
			magier.setProperty(Kampfzauber.CONFUSIONSPELL, "Steinschlag");
        ...
        for (Unit u:units) {
            ...
            if (tokens[1].equals("01")) {
                ...
                for (Message msg : messages) {
                    ...
                    if (text.contains("wurde durch den steinschlag") && text.contains("besch")) found = true; // besch...ädigt
                }
                if (!found) retval = fail(tokens[1] + ": Meldung über Steinschlagschäden fehlt.");
