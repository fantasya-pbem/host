 ...
 * @author hb
 */
public class GibBauern extends TestBase {
    ...
    protected void mySetupTest() {
        ...
        // TODO Testen, ob man den Bauern im Ozean etwas geben kann.
		{
			...
			u.Befehle.add("@GIB Bauern 1000 Kamele // dies ist ein Kommentar");
			u.Befehle.add("GIB Bauern 1000 Eisen");
			u.Befehle.add("GIB Bauern 1000 Steine");
			u.Befehle.add("GIB Bauern 2000 Holz");

			// Diese netten Leute hier wollen alles an die notleidende Bevölkerung geben.
			u = this.createUnit(p, r);
			u.setPersonen(10);
			u.setName(this.getName() + " 02");
			u.Befehle.add("GIB Bauern ALLES");
...

        {
			...
			u.Befehle.add("GIB Bauern Kamele");
			u.Befehle.add("GIB Bauern Eisen");
			u.Befehle.add("GIB Bauern Steine");
			u.Befehle.add("GIB Bauern Holz");
    ...
    protected boolean verifyTest() {
        ...
        for (Unit u:units) {
            ...
            if (tokens[1].equals("01")) {
				...
				// bei Tieren erwarten wir jeweils nur 900 statt 1000: Ein paar könnten auswandern.
				...
				if (r.getResource(Kamel.class).getAnzahl() < 900) {
				...
				for (Einzelbefehl eb : u.BefehleExperimental) {
					String b = eb.getBefehlCanonical().toLowerCase();
					// new Debug("Befehl: " + b);
...
//                if (found) retval = fail(tokens[1] + ": Skeleton nicht überschrieben.");
            ...
            if (tokens[1].equals("02")) {
                ...
                for (Message msg : messages) {
                    ...
                    if (text.contains("gib bauern alles") && text.contains("kein gültiger")) found = true;
                }
                if (!found) retval = fail(tokens[1] + ": Keine Fehlermeldung, dass GIB Bauern ALLES nicht erlaubt ist.");
			}
